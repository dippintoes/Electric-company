...
const create = (bill: IBill) => {
  if (bill.client_id)
    bill.client_id = new mongoose.mongo.ObjectId(bill.client_id);
...
const takeReading = async (id: string, files: any, bill: IBill) => {
  const client = await userServices.findOne({
    _id: new mongoose.mongo.ObjectId(bill.client_id),
  });
  const MeterType = await meterServices.findOne({
    _id: new mongoose.mongo.ObjectId(client?.meterType),
  });
  if (!MeterType)
    ...
    if (files.length === MeterType.default_pics) {
      const temp: string[] = [];
      files.forEach((img: any) => temp.push(img.filename));
      bill.pics = temp.slice(0);
    } else {
      throw {
        message:
          "Please enter : 20 pics for Normal, 8 for Commercial and 1 for Solar",
        statusCode: 400,
      };
    }
    bill.payment_status = Status.Pending;
    const exists = await billRepo.findSpecificBill(
      new mongoose.mongo.ObjectId(bill.client_id)
    );
    console.log(exists);
    if (exists.length != 0) {
    ...
    await userRepo.updateOne(bill.client_id.toString(), {
      bill: bill.totalBill,
    });
...
const updateStatus = async (client_id: string) => {
  const bill = await billRepo.findAll({
    client_id: new mongoose.mongo.ObjectId(client_id),
  });
  console.log(bill);
  if (bill.length == 0) {
    throw BILL_RESPONSES.NO_OUTSTANDING_BILL;
  } else if (
    bill[bill.length - 1].payment_status.toString() === Status.Paid.toString()
  ) {
    throw BILL_RESPONSES.ALREADY_PAID;
  } else {
    bill.forEach((item) =>
      billRepo.updateBill(item._id.toString(), {
...
const outStandingRevenue = async () => {
  console.log("start");
  const outStandingBills = await billRepo.findAll({
...
const deleteBill = async (
  filter: FilterQuery<IBill>,
  update: UpdateQuery<IBill>
) => {
