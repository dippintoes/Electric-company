...
const create = (user: IUser) => {
  if (user.role && user.role?.toString() == "ADMIN") {
...
const findAllClients = async () => {
  const allClients = await userRepo.findAll({
    role: Roles.CLIENT,
    isDeleted: false,
  });
...
const findAllEmployees = async () => {
  const allEmployees = await userRepo
    .findAll({
      role: Roles.EMPLOYEE,
      isDeleted: false,
    })
...
const getMeterRevenue = async (meterID: string) => {
  const meterUsers = await userRepo.findAll({
    meterType: new mongoose.mongo.ObjectId(meterID),
  });
